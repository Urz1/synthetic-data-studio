...
const FALLBACK_DEMO_DATASETS: DemoDataset[] = [
  {
    id: "customers",
...
const BACKEND_URL =
  process.env.NEXT_PUBLIC_BACKEND_URL || "https://api.synthdata.studio";
    ...
    const fetchDemoDatasets = async () => {
      try {
        ...
        if (res.ok) {
          ...
          const demos = data.demo_datasets || [];
          setDemoDatasets(demos.length > 0 ? demos : FALLBACK_DEMO_DATASETS);
          return;
        }
        setDemoDatasets(FALLBACK_DEMO_DATASETS);
      } catch {
  ...
  const handleSchemaGenerate = async () => {
    ...
    if (validColumns.length === 0) {
      toast({
        ...
        description: "Add at least one column with a name",
        variant: "destructive",
      });
      return;
    }
    ...
    const duplicates = validColumns.filter(
      (c) => names.filter((n) => n === c.name.toLowerCase()).length > 1
    );
    if (duplicates.length > 0) {
      toast({
        title: "Duplicate column names",
        description: `Column names must be unique: ${[
          ...new Set(duplicates.map((d) => d.name)),
        ].join(", ")}`,
        variant: "destructive",
      });
      return;
    ...
    try {
      ...
      if (response.status === 429) {
        toast({
          ...
          description:
            "Hourly limit reached. Create an account for unlimited access.",
          variant: "destructive",
        });
        return;
      }
      ...
      if (error instanceof TypeError && error.message === "Failed to fetch") {
        toast({
          ...
          description:
            "The server is not running. Please start the backend server.",
          variant: "destructive",
        });
      } else {
        toast({
  ...
  const handleMLGenerate = async () => {
    if (!selectedFile && !selectedDemo) {
      toast({
        ...
        description: "Upload a CSV file or select a demo dataset",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const response = await fetch(
        `${BACKEND_URL}/playground/train-and-generate`,
        {
          method: "POST",
          body: formData,
        }
      ...
      toast({
        title: "Download started!",
        description: `Generated ${mlNumRows.toLocaleString()} synthetic rows using ${modelType.toUpperCase()}`,
      });
    } catch (error) {
      if (error instanceof TypeError && error.message === "Failed to fetch") {
        toast({
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="border-b border-border bg-background/95 backdrop-blur sticky top-0 z-50">
        <div className="container mx-auto px-4 sm:px-6 h-14 sm:h-16 flex items-center justify-between">
          <Link
            ...
            aria-label="Go to Synth Studio homepage"
          >
      ...
      <main className="container mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        <div className="max-w-5xl mx-auto">
          ...
          >
            <TabsList className="grid w-full grid-cols-2 h-12 sm:h-14">
              <TabsTrigger
                ...
                aria-label="Define schema for instant generation"
              >
                <Wand2 className="h-4 w-4" aria-hidden="true" />
                <span className="hidden xs:inline">Define</span> Schema
                <Badge
                  variant="outline"
                  className="ml-1 hidden sm:inline-flex text-xs"
                >
                  Instant
                </Badge>
              </TabsTrigger>
              <TabsTrigger
                value="upload"
                className="gap-1 sm:gap-2 text-sm sm:text-base px-2 sm:px-4"
                aria-label="Upload data and train ML model"
              >
            ...
            <TabsContent value="schema" className="space-y-4 sm:space-y-6">
              <Alert className="bg-primary/5 border-primary/20">
                ...
                <AlertTitle className="text-primary font-medium">
                  Don't know your schema structure?
                </AlertTitle>
                <AlertDescription className="flex items-center gap-2 mt-1">
                  Use our <span className="font-semibold">AI Generator</span> to
                  ...
                  <Card>
                    <CardHeader className="pb-3 sm:pb-6">
                      ...
                      <CardDescription className="text-sm">
                        Add up to {MAX_COLUMNS} columns. Max{" "}
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="pt-4 border-t space-y-2">
                        ...
                        >
                          {[100, 500, 1000, 2500, 5000].map((preset) => (
                            <Button
                              ...
                              type="button"
                              variant={
                                numRows === preset ? "default" : "outline"
                              }
                              size="sm"
                        ...
                        />
                        <p id="row-count-hint" className="sr-only">
                      ...
                      >
                        {remainingGenerations > 0 ? (
                          <>
                            ...
                            />
                            <span>
                              {remainingGenerations} free generation
                          ...
                          <>
                            ...
                            <span>
                              ...
                              <Link
                                href="/register"
                                className="text-primary underline hover:no-underline focus:ring-2 focus:ring-primary focus:ring-offset-2 rounded"
                              >
                                Sign up
                              </Link>{" "}
                  ...
                  <Card className="bg-gradient-to-br from-primary/5 to-muted border-primary/10 overflow-hidden relative">
                    ...
                    <CardContent className="space-y-3 text-sm">
                      <div className="flex gap-2 items-start">
                        ...
                        <p>
                          Generate <strong>1M</strong> rows
                        </p>
                      </div>
                      <div className="flex gap-2 items-start">
                        <ShieldCheck className="h-4 w-4 text-muted-foreground shrink-0 mt-0.5" />
                        <p>
                          Get detailed <strong>Privacy Reports</strong>
                        </p>
                      </div>
                      <div className="flex gap-2 items-start">
                        <FileJson className="h-4 w-4 text-muted-foreground shrink-0 mt-0.5" />
                        <p>
                          Export as <strong>JSON</strong>
                        </p>
                      </div>
            ...
            <TabsContent value="upload" className="space-y-4 sm:space-y-6">
              ...
              <Alert>
                ...
                <AlertTitle>
                  Minimum {MIN_ROWS_FOR_TRAINING} rows required
                </AlertTitle>
                <AlertDescription className="text-sm">
                  ML models need sufficient data to learn patterns. For smaller
                  datasets, use the schema-based approach.
                </AlertDescription>
              </Alert>
                ...
                <div className="lg:col-span-2 space-y-4 sm:space-y-6">
                  <Card>
                    <CardHeader className="pb-3 sm:pb-6">
                      ...
                      <CardDescription className="text-sm">
                        Train a model on your data and generate a synthetic
                        twin.
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4 sm:space-y-6">
                      ...
                      <div className="space-y-3">
                        ...
                        <div
                          ...
                          aria-label={
                            selectedFile
                              ? `Selected file: ${selectedFile.name}. Press Enter to change file.`
                              : "Click or press Enter to upload a CSV file"
                          }
                        >
                          <input
                            ref={fileInputRef}
                          ...
                          {selectedFile ? (
                            <div className="space-y-2">
                              <FileSpreadsheet
                                ...
                                aria-hidden="true"
                              />
                              <p className="font-medium text-sm sm:text-base truncate max-w-[200px] mx-auto">
                                {selectedFile.name}
                              </p>
                            ...
                            <div className="space-y-2">
                              <Upload
                                className="h-8 w-8 sm:h-10 sm:w-10 mx-auto text-muted-foreground"
                                aria-hidden="true"
                              />
                              ...
                              >
                                Max 5MB, min {MIN_ROWS_FOR_TRAINING} rows
                              </p>
                            </div>
                          )}
                        </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <p id="model-hint" className="sr-only">
                          CTGAN is best for mixed data types, TVAE is best for
                          numerical data
                        </p>
                      </div>
                      ...
                      <div className="rounded-lg border border-border bg-muted/40 p-3 space-y-3">
                        ...
                        <Button
                          variant="link"
                          className="h-auto p-0 text-xs text-primary w-full justify-start"
                          asChild
                        >
                          <Link href="/register">
                            Sign up to unlock privacy safeguards
                          </Link>
                        </Button>
                      </div>
                        ...
                        >
                          {[100, 500, 1000, 2500, 5000].map((preset) => (
                            <Button
                              ...
                              type="button"
                              variant={
                                mlNumRows === preset ? "default" : "outline"
                              }
                              size="sm"
                      ...
                      {loading && (
                        ...
                        >
                          ...
                          <p className="text-xs text-muted-foreground">
                            This may take 30s-2min depending on data size
                          </p>
                        </div>
                      )}
          ...
          {generationComplete && (
            <Card
              className="border-primary/50 bg-primary/5 mt-6"
              role="alert"
              aria-live="polite"
            >
                ...
                <div className="flex flex-col sm:flex-row items-start gap-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Want to save your datasets, train longer, and generate up
                      to 1M rows?
                    </p>
                    <Button
                      asChild
                      className="font-semibold transition-all duration-200 hover:scale-[1.02] active:scale-[0.98]"
                    >
                      <Link href="/register">
                        Create Free Account{" "}
                        <ArrowRight
          ...
          <div className="space-y-4 py-4">
            <div className="p-4 bg-primary/5 rounded-lg border border-primary/10">
              ...
              <p className="text-sm text-muted-foreground mb-3">
                Our <strong>AI-Enhanced Generator</strong> understands context
                and relationships better than simple rules.
              </p>
