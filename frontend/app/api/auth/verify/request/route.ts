...
export async function POST(request: Request) {
  ...
  if (!email) {
    const redirect = `/verify-email?error=${encodeURIComponent(
      "Email is required"
    )}`;
  ...
  const backendRes = await fetch(`${API_BASE}/auth/verify/request`, {
    method: "POST",
  ...
  const redirect = `/verify-email?sent=1&email=${encodeURIComponent(email)}`;
  if (wantsJson(request)) {
