...
export function DatasetProfileView({
  ...
  const columns: ColumnInfo[] = React.useMemo(() => {
    ...
    if (dataset.schema_data) {
      if (
        "dtypes" in dataset.schema_data &&
        typeof dataset.schema_data.dtypes === "object"
      ) {
    ...
    return columnNames.map((colName: string) => {
      ...
      const dtype =
        typeof typesVal === "string" ? typesVal : JSON.stringify(typesVal);
    ...
    },
    {
      ...
      accessor: (row: ColumnInfo) => (
        <span
          className={cn(
            "text-sm font-mono",
            (row.profile?.missing_percent || 0) > 10 &&
        ...
        if (row.profile.type === "categorical") {
          return (
            <span className="text-xs text-muted-foreground">
              {row.profile.unique_count} unique values
            </span>
          );
        }
      ...
      accessor: (row: ColumnInfo) => {
        ...
        return (
          <Badge
            ...
            className={cn(
              "text-xs",
              row.piiFlag.confidence > 0.9 &&
                "bg-risk/10 text-risk border-risk/20",
            )}
          >
            {row.piiFlag.pii_type}
            <span className="ml-1 opacity-60">
              {(row.piiFlag.confidence * 100).toFixed(0)}%
            </span>
          </Badge>
        );
      },
      ...
      <CardHeader className="pb-2 sm:pb-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          ...
          {piiColumns.length > 0 && (
            <Badge
              variant="secondary"
              className="bg-warning/10 text-warning-foreground border-warning/20 w-fit text-xs"
            >
              ...
              <div className="space-y-4">
                <div className="rounded-lg border border-warning/20 bg-warning/5 p-4">
                  <p className="text-sm text-warning-foreground">
                    Detected potentially sensitive columns. Consider excluding
                    or transforming these before generating synthetic data.
                  </p>
                </div>
                      ...
                      <div key={col.name} className="rounded-lg border p-4">
                        ...
                        <div className="flex items-center justify-between text-sm">
                          ...
                          <span className="font-mono">
                            {(col.piiFlag!.confidence * 100).toFixed(0)}%
                          </span>
                        </div>
                        {col.piiFlag!.sample_matches && (
                          <div className="mt-2 pt-2 border-t">
                            ...
                            <div className="flex flex-wrap gap-1 mt-1">
                              {col
                                .piiFlag!.sample_matches.slice(0, 3)
                                .map((match, i) => (
                                  <code
                                    key={i}
