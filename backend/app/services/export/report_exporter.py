...
class ReportExporter:
    ...
    def _load_logo_base64(self) -> Optional[str]:
        """Load the company logo and convert to base64 for embedding in reports."""
        try:
            logo_path = self.templates_dir / "logo.svg"
            if logo_path.exists():
    ...
    def export_to_pdf(
        ...
        document_type: str = "Compliance Report",
        ...
        Args:
            ...
            document_type: Type of document (Model Card, Privacy Report, etc.)
            classification: Document classification (Internal, Confidential, Restricted)
        ...
        try:
            ...
            doc_id = datetime.datetime.now().strftime("%Y%m%d%H%M%S")
    ...
    def export_to_docx(
        ...
        try:
            ...
            p.text = f"Confidential - Synth Data Studio | support@synthdata.studio | {datetime.datetime.now().year}"
            p.alignment = WD_ALIGN_PARAGRAPH.CENTER
