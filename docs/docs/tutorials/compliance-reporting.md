---
id: tutorials-compliance-reporting
title: "Compliance Reporting Tutorial"
...
```bash
curl -X POST "https://api.synthdata.studio/generators/{generator_id}/compliance-report" \
  -H "Content-Type: application/json" \
  -d '{"framework": "HIPAA"}'
```
...
```bash
curl -X POST "https://api.synthdata.studio/generators/{generator_id}/model-card" \
  -H "Content-Type: application/json" \
...
```bash
curl https://api.synthdata.studio/generators/{generator_id}/audit-narrative
```
...
curl https://api.synthdata.studio/compliance/health-check
```
